...
//OPTIONS SETUP MENU CLICK	
	...
	var total_steps = jQuery('.nex-forms-container .form_field.step').size();
		...
		setTimeout(function(){ jQuery('.multi-step-tools ul li:eq(1)').find('a').trigger('click'); },200);
		set_paypal_fields();		
		set_mc_field_map();
		set_gr_field_map();
		set_ftp_field_map();
	...
	jQuery(document).on('click','.multi-step-tools ul a', //.nf_step_breadcrumb a,
		function()
			{
...
function nf_count_multi_steps(){
	...
	jQuery('.multi-step-stepping').html(set_breadcrumb);
