		...
		public function builder7_top_menu(){
				...
				$output .= '<div class="row row_zero_margin menu_wrapper ">';
					$output .= '
						...
						$output .= '<li class="expand_fullscreen" style="display:none"><a class="btn waves-effect waves-light btn-fullscreen" href="#" data-toggle="tooltip_bs" data-placement="bottom" title="'.__('Enter Full Screen Mode','nex-forms').'"><span class="fa fas fa-expand-arrows-alt"></span></a></li>';
						$output .= '<li class="colapse_fullscreen"><a class="btn waves-effect waves-light btn-wordpress" href="#" data-toggle="tooltip_bs" data-html="true" data-placement="bottom" title="'.__('Exit Fullscreen Mode<br>Show WordPress Menus','nex-forms').'"><span class="fa fa-wordpress"></span></a></li>';
		...
		public function print_overall_settings(){
			...
			$output .= '<div class="overall-settings-column overall-form-styling-column settings-column-style right_hand_col">';
					...
					';
						...
						$output .= '<div class="material_box_head"><span class="fa fa-paint-brush"></span>'.__('Overall Styling','nex-forms').'</div>';
...
//LABEL SETTINGS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
						...
						$output .= '<div id="form-settings-panel" class="form-settings settings-section active">';
							...
							$output .= '<select name="google_fonts_overall" id="google_fonts_overall" class="sfm form-control"><option value="">'.__('Default','nex-forms').'</option>';
		...
		public function print_field_settings(){
			...
			$output .= '<div class="field-settings-column settings-column-style right_hand_col ">';
					...
					<div class="current_id" style="display:none;"></div>';
						...
						$output .= '<div class="material_box_head"><span class="fa fa-edit"></span>'.__('Edit Field','nex-forms').' <span class="set_editing_field"></span></div>';
			...
			$output .= '<div class="form-canvas-area">';
				...
				$output .= '<div class="canvas-tools multi-step-tools"><span class="tool-label">'.__('Multi-Step','nex-forms').'<span class="fa fa-caret-right"></span></span>';
					...
					$output .= '<div class="tool-section ">';
						$output .= '<ul class="show_all_steps" style="display:none;"><li class="all_steps" data-toggle="tooltip_bs" data-placement="bottom" title="'.__('Show all Steps','nex-forms').'"><a data-show-step="all">'.__('All','nex-forms').' <span class="all_steps_count"></span></a></li></ul>';
				...
				$output .= '<div class="canvas-tools hidden">';
					...
					$output .= '<span class="tool-label top-tools">'.__('Theme','nex-forms').'</span>
					...
					$output .= '<select name="md_theme_shade_selection" class="form-control md_theme_shade_selection" data-selected="'.$set_theme_shade.'">
									<option value="light" 	'.(($set_theme_shade=='light' || !$set_theme_shade) ? 'selected="selected"' : '').'>'.__('Light','nex-forms').'</option>
									<option value="dark" 	'.(($set_theme_shade=='dark') ? 'selected="selected"' : '').'>'.__('Dark','nex-forms').'</option>
								</select>';
																...
																switch($attr['type'])
																	...
																	case 'submit-button':
																		$output .= '<div class="col-sm-12  input_container">';
																			$output .= '<button class="nex-submit svg_ready the_input_element btn btn-default" onclick="'.$this->google_analytics_conversion_code.'">Submit</button>';
																		$output .= '</div>';
																		$i=0;
																	break;
																	case 'submit-button2':
																		$output .= '<div class="col-sm-12  input_container">';
																			$output .= '<button class="nex-submit svg_ready the_input_element btn btn-default" onclick="'.$this->google_analytics_conversion_code.'">'.$attr['label'].'</button>';
																		$output .= '</div>';
...
tiff</div>
												...
												$output .= '<div class="row">';
													$output .= '<div class="col-sm-12 input_container">';
															...
															switch($attr['type'])
																...
																break;
																case 'submit-button':
																	$output .= '<button class="nex-submit svg_ready the_input_element btn btn-default" onclick="'.$this->google_analytics_conversion_code.'">'.__('Submit','nex-forms').'</button>';
		...
		//6 Columns								
				...
				$output .= '<a class="toolbar-action-btn conditional-logic-btn btn" title="'.__('Conditional Logic','nex-forms').'"><span class="fa fa-random"></span>'.__('Conditional Logic','nex-forms').'</a>';
				$output .= '<a class="toolbar-action-btn overall-styling-btn btn" title="'.__('Overall Settings','nex-forms').'"><span class="fa fa-paint-brush"></span>  '.__('Overall Styling','nex-forms').'</a>';
				...
				$output .= '</div>';
					...
					$output .= '<div class="nf_step_breadcrumb " style="display:none;">
									<ol class="'.$bc_type.' '.(($bc_type=='basix' || $bc_type=='triangular') ? 'cd-breadcrumb' : 'cd-multi-steps').'  '.(($bc_type=='rectangular') ? 'text-center' : '').' '.(($bc_type=='triangular') ? 'triangle' : '').' '.(($bc_type=='dotted_count') ? 'count' : '').' '.(($bc_type=='dotted' || $bc_type=='dotted_count') ? $bc_text_pos : '').' md-color-'.$bc_data_theme.'" data-text-pos="'.$bc_text_pos.'" data-breadcrumb-type="'.$bc_type.'" data-theme="'.$bc_data_theme.'" data-show-front-end="'.$bc_show_front_end.'" data-show-inside="'.$bc_show_inside.'"></ol>
								</div>';
							...
							echo '<div class="email_setup_wrapper">';
									...
									echo '<div class="row">';
										...
										echo '<div class="integration_form_field tut_admin_email_3">';
											echo '<input type="text" class="form-control" name="nex_autoresponder_recipients" id="nex_autoresponder_recipients"  placeholder="'.__('Example: email@domain.com, email2@domain.com','nex-forms').'" value="'.(($this->mail_to) ? str_replace('\\','',$this->mail_to) : $preferences['email_preferences']['pref_email_recipients']).'">';
										echo '</div>';
									echo '</div>';
	...
	public function new_form_wizard(){
		$output = '<div id="new_form_wizard" class="modal">
								<div class="modal-header">
									<h4>'.__('Create a new form','nex-forms').'</h4>
								...
								<div class="wizard_step start_new_form" style="display:none;">
								 ...
								 <div class="wizard_step start_new_import" style="display:none;">
									...
									<div class="col-xs-12">
										<div class="alert alert-info" id="upload_form2">'.__('Browse to an Exported NEX-Form and Click Open','nex-forms').'</div>
									 </div>  
								 </div>
