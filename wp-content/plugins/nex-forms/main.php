...
function NEXForms5_main_menu(){
	...
	add_submenu_page( 'nex-forms-dashboard', 'NF-Builder','Builder', $nf_user_level, 'nex-forms-test-page-2', 'NEXForms_test_page_2');
...
function NEXForms_form_preview(){
	...
	echo '<style type="text/css">
		...
		#nex-forms .ui-nex-forms-container .label_container label{
		...
		div.updated, .update-nag , div.error, #icl_als_help_popup, .wd-notice-body, .notice,.notice-warning,.mom_modal_box, .notice, #dolly,.cts-tip-wrapper{
	...
	if($theme_settings['0']['theme_shade']=='dark')
		{
		echo '<style type="text/css">
				html, body, #wpcontent, #wpwrap, #wpbody-content  {
		...
		if($open_trigger=="popup")
			...
			if($exit_intent==1)
				...
				if($type == 'custom_trigger')
					...
					$output .= '<div id="nex-forms"><button class="btn mb-btn waves-effect waves-light '.$button_color.' open_nex_forms_popup" data-popup-id="'.$atts['id'].'">'.$text.'</button></div>';
				else
...
//#NEX-FORMS		
		$output .= '<div id="nex-forms" class="nex-forms '.$nf_functions->format_name($form_attr->title).' '.(($make_sticky=='yes') ? 'is-nf-sticky-form' : '').'">';
			...
			$output .= '<div class="theme-'.$set_theme.'">';
			...
			$output .= '<div id="on_form_submmision" class="on_form_submmision" style="display:none;">'.$form_attr->on_form_submission.'</div>';
				...
				<div class="msg_box"><div class="msg_text" style="display:none;">'.str_replace('\\','',$form_attr->on_screen_confirmation_message).'</div></div>
						...
						if($paypal_transaction==1)
							...
							if($payment_details['payment_verified'])
								...
								$output .= '<div class="nex_success_message animated">
									...
									<div class="success_header">
									<div class="success_icon animated"><span class="fa fa-check"></span></div>
									</div>
									<div class="msg_box"><div class="msg_text" >'.str_replace('\\','',$form_attr->payment_success_msg).'</div></div>
	...
	$css_js_version = $config->plugin_version.'.23';
...
/* SCRIPTS AND STYLE INCLUSIONS */		
	...
	if($script_config['inc-bootstrap']=='1' || !$script_config['inc-bootstrap'])
		wp_enqueue_script('nex-forms-bootstrap.min',  plugins_url( '/js/bootstrap.min.js?v'.$css_js_version.'',__FILE__));

		
	wp_enqueue_script('nex-forms-math.min',  plugins_url( '/js/math.min.js',__FILE__));	
	
	wp_enqueue_script('nex_forms-materialize.min',  plugins_url( '/nf-admin/js/materialize.js?v'.$css_js_version.'',__FILE__));	
	
	
	
		if($script_config['inc-wow']=='1' || !$script_config['inc-wow'])
			{
			wp_enqueue_script('nex-forms-wow',  plugins_url( '/js/wow.min.js?v'.$css_js_version.'',__FILE__));
	...
	if($script_config['inc-raty']=='1' || !$script_config['inc-raty'])
		{
		wp_enqueue_script('nex-forms-raty-fa',  plugins_url( '/js/jquery.raty-fa.js?v'.$css_js_version.'',__FILE__));
	...
	if($script_config['inc-onload']=='1' || !$script_config['inc-onload'])
		wp_enqueue_script('nex-forms-onload', plugins_url( '/js/nexf-onload-ui.js?v'.$css_js_version.'',__FILE__));
	

	wp_enqueue_script('nf-jquery-ui',plugins_url( '/js/jquery-ui.js?v'.$css_js_version.'',__FILE__));
	...
	if($styles_config['incstyle-font-awesome']=='1' || !$script_config['incstyle-font-awesome'])
		{
		wp_enqueue_style('nex-forms-font-awesome-5',plugins_url( '/css/fa5/css/all.min.css?v'.$css_js_version.'',__FILE__));
		wp_enqueue_style('nex-forms-font-awesome-4-shims',plugins_url( '/css/fa5/css/v4-shims.min.css?v'.$css_js_version.'',__FILE__));
		}
	if($styles_config['incstyle-bootstrap']=='1' || !$script_config['incstyle-bootstrap'])
		wp_enqueue_style('nex-forms-bootstrap-ui', plugins_url( '/css/ui-bootstrap.css?v'.$css_js_version.'',__FILE__));

		wp_enqueue_style('nex-forms-ui', plugins_url( '/css/ui.css?v7.2.7',__FILE__));

		wp_enqueue_style('nex-forms-fields', plugins_url( '/css/fields.css?v'.$css_js_version.'',__FILE__));
	if($styles_config['incstyle-animations']=='1' || !$script_config['incstyle-animations'])
		wp_enqueue_style('nex-forms-animations', plugins_url( '/css/animate.css?v'.$css_js_version.'',__FILE__));
	

		wp_enqueue_script('nex-forms-moment.min', plugins_url( '/js/moment.min.js?v'.$css_js_version.'',__FILE__));

		wp_enqueue_script('nex-forms-locales.min', plugins_url( '/js/locales.js?v'.$css_js_version.'',__FILE__));	

		wp_enqueue_script('nex-forms-bootstrap-datetimepicker', plugins_url( '/js/bootstrap-datetimepicker.js?v'.$css_js_version.'',__FILE__));
		
		wp_enqueue_script('bootstrap-material-datetimepicker', plugins_url( '/js/bootstrap-material-datetimepicker.js?v'.$css_js_version.'',__FILE__));
		wp_enqueue_style('bootstrap-material-datetimepicker', plugins_url( '/css/bootstrap-material-datetimepicker.css?v'.$css_js_version.'',__FILE__));
		
		wp_enqueue_script('jqui-timepicker', plugins_url( '/js/jqui-timepicker.js?v'.$css_js_version.'',__FILE__));
		wp_enqueue_style('jqui-timepicker', plugins_url( '/css/jqui-timepicker.css?v'.$css_js_version.'',__FILE__));
		
		wp_enqueue_script('nex-forms-bootstrap-touchspin', plugins_url( '/js/jquery.bootstrap-touchspin.js?v'.$css_js_version.'',__FILE__));
		wp_enqueue_style('nex-forms-bootstrap-touchspin', plugins_url( '/css/jquery.bootstrap-touchspin.css?v'.$css_js_version.'',__FILE__));
	
	
		wp_enqueue_script('nex-forms-auto-save', plugins_url( '/js/inputStore.jquery.js?v'.$css_js_version.'',__FILE__));
	...
	wp_enqueue_style('nouislider', plugins_url( '/css/nouislider.css?v'.$css_js_version.'',__FILE__));
	wp_enqueue_style('nex-forms-materialize', plugins_url( '/css/materialize-ui.css?v'.$css_js_version.'',__FILE__));	
	wp_enqueue_style('nf-md-checkbox-radio', plugins_url( '/css/material-checkboxradio.css?v'.$css_js_version.'',__FILE__));	
		...
		foreach($_POST as $key=>$val)
			...
			$key!='submit' &&
 			!strstr($key,'real_val')
				...
				{
				$data_array[] = array('field_name'=>esc_html($key),'field_value'=>esc_html($val));
				$i++;
				}
			}		
		$user_fields .= '</table>';
...
function NEXForms_form_builder(){
		...
		echo '<div id="nex-forms">';
		 ...
		 echo '<div class="nex_forms_admin_page_wrapper fullscreen">';
...
function nf_send_mail($nex_forms_id='', $entry_id='', $resent=0,$send_email=true, $files=array()){
	...
	/*******************************************************************************************************/
			...
			$user_fields 	= '<table width="100%" cellpadding="5" cellspacing="0" style="border-top:1px solid #ddd; border-right:1px solid #ddd; border-left:1px solid #ddd;">';
			$user_fields .= '<tr>
							<td  valign="top" style="padding-top:10px; padding-bottom:10px; border-bottom:1px solid #ddd; background-color:#f9f9f9;" colspan="2"><strong>'.__('Submitted Form Data:','nex-forms').'</strong></td>
						<tr>
						';
			$i				= 1;
					...
					if($val!='NaN')
						...
						if(is_array($val))
							...
							$user_fields .= '<tr>
												...
												<td width="85%" style="border-bottom:1px solid #ddd;" valign="top">'.nl2br(str_replace('\\','',$admin_val)).'</td>
											<tr>
											';
							}
						else
		...
		$paypal_data 	= '<br /><table width="100%" cellpadding="5" cellspacing="0" style="border-top:1px solid #ddd; border-right:1px solid #ddd; border-left:1px solid #ddd;">';
		$paypal_data .= '<tr>
							<td  valign="top" style="padding-top:10px; padding-bottom:10px; border-bottom:1px solid #ddd; background-color:#f9f9f9;" colspan="2"><strong>'.__('PayPal Payment Detials:','nex-forms').'</strong></td>
						<tr>
						';
		$paypal_data .= '<tr>
							<td width="30%" valign="top" style="border-bottom:1px solid #ddd;border-right:1px solid #ddd; background-color:#f9f9f9;"><strong>'.__('Status:','nex-forms').'</strong></td>
							<td width="70%" style="border-bottom:1px solid #ddd;" valign="top"><span class="payment_status">'.$entry_attr->payment_status.'</span></td>
						<tr>
						';
		$paypal_data .= '<tr>
							<td width="30%" valign="top" style="border-bottom:1px solid #ddd;border-right:1px solid #ddd; background-color:#f9f9f9;"><strong>'.__('Ammount:','nex-forms').'</strong></td>
							<td width="70%" style="border-bottom:1px solid #ddd;" valign="top">'.$entry_attr->payment_ammount.'</td>
						<tr>
						';
		$paypal_data .= '<tr>
							<td width="30%" valign="top" style="border-bottom:1px solid #ddd;border-right:1px solid #ddd; background-color:#f9f9f9;"><strong>'.__('Currency:','nex-forms').'</strong></td>
							<td width="70%" style="border-bottom:1px solid #ddd;" valign="top">'.$entry_attr->payment_currency.'</td>
						<tr>
						';
		$paypal_data .= '<tr>
							<td width="30%" valign="top" style="border-bottom:1px solid #ddd;border-right:1px solid #ddd; background-color:#f9f9f9;"><strong>'.__('Payment ID:','nex-forms').'</strong></td>
							<td width="70%" style="border-bottom:1px solid #ddd;" valign="top">'.$entry_attr->paypal_payment_id.'</td>
						<tr>
						';
		$paypal_data .= '<tr>
							<td width="30%" valign="top" style="border-bottom:1px solid #ddd;border-right:1px solid #ddd; background-color:#f9f9f9;"><strong>'.__('Payment Token:','nex-forms').'</strong></td>
							<td width="70%" style="border-bottom:1px solid #ddd;" valign="top">'.$entry_attr->paypal_payment_token.'</td>
						<tr>
						';
